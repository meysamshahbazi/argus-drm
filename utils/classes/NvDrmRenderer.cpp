...
static int NvBufGetDrmParams(NvBufferParams *params, NvBufDrmParams *dParams)
{
  ...
  switch (params->pixel_format) {
    case NvBufferColorFormat_ABGR32:
      dParams->pixel_format = DRM_FORMAT_ABGR8888;
      break;
...
NvDrmRenderer::NvDrmRenderer(const char *name, uint32_t w, uint32_t h,
                    uint32_t w_x, uint32_t w_y, uint32_t aconn, uint32_t acrtc,uint32_t plane_id,
        ...
        :NvElement(name, valid_fields)
{
  ...
  // printf("drm_conn_info->count_encoders: %d\n",drm_conn_info->count_encoders);
  if (!drm_enc_info) {
  ...
  if (hdrSupported()) {
    std::cout << "*****************hdrSupported\n";
    ret = setHDRMetadataSmpte2086(metadata);
    if(ret!=0){
      std::cout <<"Error while getting HDR mastering display data\n";
  ...
  // create_rt_pthread(&render_thread, &renderThread,(void *)this);
...
NvDrmRenderer::drmUtilCloseGemBo (int fd, uint32_t bo_handle)
{
  ...
  if (ret < 0) {
    std::cout << "Failed to close gem buffer\n" << std::endl;
    return 0;
  }
  return 1;
}
                                      ...
                                      unsigned int sec, unsigned int usec, void *data)
{
  ...
  if (renderer->pendingBuffers.empty()) {
    ...
    // std::cout <<  renderer->pendingBuffers.size() << std::endl;
  ...
  if (map_entry != map_list.end()) {
   ...
   for (i = 0; i < dParams.num_planes; i++) {
    ...
    if (planeIsSet == false)
    {
      ...
      ret = setPlane(m_plane_id, fb, 0, 0, width, height, 0, 0, width, height);
...
NvDrmRenderer::createDrmRenderer(const char *name, uint32_t width,
                               ...
                               uint32_t connector, uint32_t crtc, uint32_t plane_id,
                               struct drm_tegra_hdr_metadata_smpte_2086 metadata,
                               bool streamHDR)
{
  if (!width || ! height) {
  ...
  NvDrmRenderer* renderer = new NvDrmRenderer(name, width, height, w_x, w_y, connector, crtc, plane_id, metadata, streamHDR);
